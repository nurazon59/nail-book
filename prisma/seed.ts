...
async function main() {
	const brandNames = ['PreGel', 'T-Gel', 'Kokoist', 'emena', 'toys'];
		...
		nailSets.map((nailSet) =>
			Promise.all(
				fingerNames.map((fingerName) => {
					const randomBaseCoat =
						baseCoats[Math.floor(Math.random() * baseCoats.length)];
					const randomTopCoat =
						topCoats[Math.floor(Math.random() * topCoats.length)];
					const randomArtNail =
						artNails[Math.floor(Math.random() * artNails.length)];
					const randomColorNail =
						colorNails[Math.floor(Math.random() * colorNails.length)];
	...
	console.log(
		'指ごとに異なるブランドを使ったシードデータの作成が完了しました！',
	);
}
